...
def evaluate(epoch):
    ...
    for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc='Evaluation'):
        step += 1
        input_ids, attention_mask = pad_to_window_size(input_ids, input_mask, 512, tokenizer.pad_token_id)
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                print('longformer-model*****epoch:{}, step:{}, loss:{:10f}, time_cost:{:10f}'.format(epoch, step, loss, time.time()-start_time))
                if args.fp16:
