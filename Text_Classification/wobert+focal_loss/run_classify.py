# -*- encoding: utf-8 -*-
'''
@File    :   run_classify_loss.py
@Time    :   2020/10/23 13:22:26
@Author  :   xiaolu 
...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        pred = pred.view(-1,1)   # size: batch_size, 1
        target = target.view(-1,1)    # size: batch_size, 1
        # print(pred.size())    # torch.Size([256, 1])
        # print(target.size())   # torch.Size([256, 1])
        ...
        class_mask = torch.zeros(pred.size(0), pred.size(1)).cuda()   
        class_mask.scatter_(1, target.view(-1, 1).long(), 1.)    # 将target转为one_Hot

        # 利用 mask 将所需概率值挑选出来
        probs = (pred * class_mask).sum(dim=1).view(-1,1)
        probs = probs.clamp(min=0.0001,max=1.0)    # 预测为正样本和负样本的概率

        # 计算概率的 log 值
        log_p = probs.log()

        # 根据论文中所述，对 alpha　进行设置（该参数用于调整正负样本数量不均衡带来的问题）
        alpha = torch.ones(pred.shape[0],pred.shape[1]).cuda()  # batch_size, 2
...
def evaluate(epoch, eval_features, args, model):
    ...
    print(s)
    s += '\n'
    with open('result_eval_bceloss.txt', 'a+') as f:
        f.write(s)
