...
def evaluate(epoch):
    ...
    eval_input_ids = torch.tensor([f.input_ids for f in train_features], dtype=torch.long)
    eval_bigram = torch.tensor([f.bigram for f in train_features], dtype=torch.long)
    eval_trigram = torch.tensor([f.trigram for f in train_features], dtype=torch.long)
    eval_seq_len = torch.tensor([f.seq_len for f in train_features], dtype=torch.long)
    eval_label = torch.tensor([f.label for f in train_features], dtype=torch.long)
...
if __name__ == '__main__':
    ...
    if args.do_train:
        ...
        all_bigram = torch.tensor([f.bigram for f in train_features], dtype=torch.long)
        all_trigram = torch.tensor([f.trigram for f in train_features], dtype=torch.long)
        all_seq_len = torch.tensor([f.seq_len for f in train_features], dtype=torch.long)
        all_label = torch.tensor([f.label for f in train_features], dtype=torch.long)
        
        train_data = TensorDataset(all_input_ids, all_bigram, all_trigram, all_seq_len, all_label)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                input_ids, bigram, trigram, seq_len, label = batch
                logits = model(input_ids, bigram, trigram, seq_len, label)
                ...
                print('fasttext-model*****epoch:{}, step:{}, loss:{:10f}, time_cost:{:10f}'.format(epoch, step, loss, time.time()-start_time))
