...
class FastBERTGraph(nn.Module):
    def __init__(self, bert_config, op_config):
        ...
        self.layer_classifier = FastBERTClassifier(bert_config, op_config)  # 加的分类层是dense+selfAttn+dense+dense
        self.layer_classifiers = nn.ModuleDict()
        for i in range(bert_config.num_hidden_layers - 1):   # 这里加11个分支分类层   第12个分类层就是最后的分类层
            self.layer_classifiers['branch_classifier_'+str(i)] = copy.deepcopy(self.layer_classifier)

        self.layer_classifiers['final_classifier'] = copy.deepcopy(self.layer_classifier)   # 第十二个分类层
    ...
    def forward(self, hidden_states, attention_mask, labels=None, inference=False, inference_speed=0.5, training_stage=0):
        ...
        if inference:
            ...
            for i, (layer_module, (k, layer_classifier_module)) in enumerate(zip(self.layers, self.layer_classifiers.items())):
                ...
                # 进行预测   然后进行不确定的计算 概率x概率的对数 除 类别的对数
                ...
                # 提前返回结果
                if uncertain < inference_speed:   # 如果不确定度低于我们设定的目标 则返回
        ...
        # ----训练阶段, 第一阶段初始训练, 第二阶段蒸馏训练--------
            ...
            # 蒸馏训练，每层的student和teacher的KL散度作为loss
                ...
                all_encoder_layers = []
                for layer_module in self.layers:
                    hidden_states = layer_module(hidden_states, attention_mask)   # 把bert中每一层的输出都保留下来
                ...
                loss = 0.0
                teacher_log_prob = F.log_softmax(all_logits[-1], dim=-1)   # 最后一个分类器就是老师预测的结果
                for student_logits in all_logits[:-1]:    # 得到其他分类器的结果
