...
def train(model, train_list):
    ...
    optimizer = transformers.AdamW(model.parameters(), lr=Config.lr, correct_bias=True)
    scheduler = transformers.get_linear_schedule_with_warmup(optimizer, num_warmup_steps=Config.warmup_steps, num_training_steps=total_steps)
        ...
        for batch_idx, input_ids in enumerate(train_dataloader):
            ...
            if batch_idx + 1 % Config.gradient_accumulation == 0:
                ...
                if overall_step + 1 % Config.log_step == 0:
                    print('epoch:{}, step:{}, loss: {}, accuracy:{}'.format(epoch+1, batch_idx+1, loss, accuracy))
