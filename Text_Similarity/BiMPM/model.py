...
class Model(nn.Module):
    def __init__(self, embeddings, hidden_size=100, num_perspective=20, class_size=2):
        super(Model, self).__init__()
        self.embeds_dim = embeddings.shape[1]
        self.hidden_size = hidden_size    # lstm's hidden dim
        self.l = num_perspective
        self.class_size = class_size

        # ----- Word Representation Layer -----
        self.word_emb = nn.Embedding(embeddings.shape[0], embeddings.shape[1])
        self.word_emb.weight = nn.Parameter(torch.from_numpy(embeddings).float(), requires_grad=True)

        # ----- Context Representation Layer -----
        self.context_LSTM = nn.LSTM(input_size=self.embeds_dim, hidden_size=self.hidden_size, num_layers=1,
    ...
    def forward(self, q1, q2):
        ...
        p_encode = self.dropout(p_encode)    # torch.Size([2, 40, 300])
        h_endoce = self.dropout(h_endoce)    # torch.Size([2, 40, 300])

        # ----- Context Representation Layer -----
        con_p, _ = self.context_LSTM(p_encode)   # p和h共享这个lstm
        con_h, _ = self.context_LSTM(h_endoce)
        con_p = self.dropout(con_p)    # torch.Size([2, 40, 200])
        con_h = self.dropout(con_h)    # torch.Size([2, 40, 200])
        con_p_fw, con_p_bw = torch.split(con_p, self.hidden_size, dim=-1)   # 前向和后向分开
        con_h_fw, con_h_bw = torch.split(con_h, self.hidden_size, dim=-1)   # 两个维度都为: torch.Size([2, 40, 100])

        # 1. Full-Matching
        mv_p_full_fw = mp_matching_func(con_p_fw, con_h_fw[:, -1, :], self.mp_w1, self.l)  # h正向的最后一步和p的正向每一步计算匹配得分
        mv_p_full_bw = mp_matching_func(con_p_bw, con_h_bw[:, 0, :], self.mp_w2, self.l)   # h反向的最后一步和p的反向每一步计算匹配得分
        mv_h_full_fw = mp_matching_func(con_h_fw, con_p_fw[:, -1, :], self.mp_w1, self.l)  # p正向的最后一步和h的正向每一步计算匹配得分
        mv_h_full_bw = mp_matching_func(con_h_bw, con_p_bw[:, 0, :], self.mp_w2, self.l)   # p反向的最后一步和h的方向每一步计算匹配得分
...
def mp_matching_func(v1, v2, w, l=20):
    ...
    '''
    seq_len = v1.size(1)
    w = w.transpose(1, 0).unsqueeze(0).unsqueeze(0)  # (1, 1, hidden_size, l)

    # (batch, seq_len, hidden_size, l)
    v1 = w * torch.stack([v1] * l, dim=3)  # (1, 1, hidden_size, l) * (batch_size, seq_len, hidden_size, l)
    # print(v1.size())   # torch.Size([2, 40, 100, 20])
