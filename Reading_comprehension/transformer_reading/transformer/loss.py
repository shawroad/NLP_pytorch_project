...
def cal_performance(pred, gold, smoothing=0.0):
    ...
    pred = pred.max(1)[1]
    non_pad_mask = gold.ne(Config.IGNORE_ID)
...
def cal_loss(pred, gold, smoothing=0.0):
    ...
    if smoothing > 0.0:
        ...
        gold_for_scatter = gold.ne(Config.IGNORE_ID).long() * gold
        ...
        loss = F.cross_entropy(pred, gold, ignore_index=Config.IGNORE_ID, reduction='mean')
