...
def convert_examples_to_features(data, tokenizer, max_seq_length, max_query_length):
    ...
    end = data['end_position'].tolist()

    features = []
    id = 0
    for ctext, ques, ans, start_position, end_position in zip(context, question, answer, start, end):
        id += 1
...
if __name__ == '__main__':
    # 1. 将文章转为id用的
    # tokenizer = RobertaTokenizer.from_pretrained('vocab.txt')
    tokenizer = BertTokenizer.from_pretrained('./roberta_large_pretrain/vocab.txt')

    # 2. 加载抽取的数据集
    data = pd.read_csv('./data/correct_data.csv')
    # 3. 将含有nan的行去除掉
    data = data.dropna(axis=0)
    print(data.shape[0])   # 10124
