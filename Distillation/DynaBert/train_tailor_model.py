# -*- encoding: utf-8 -*-
'''
@File    :   train_tailor_model.py
@Time    :   2020/10/19 10:22:09
@Author  :   xiaolu 
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for width_mult in sorted(args.width_mult_list, reverse=True):
                ...
                loss, student_logit, student_rep, _, _ = model(**inputs)
                ...
                s += 'width={}: {:4f}, '.format(width_mult, loss)
                if args.n_gpu > 1:
                    loss = loss.mean()
                if args.gradient_accumulation_steps > 1:
                    loss = loss / args.gradient_accumulation_steps
                loss.backward()
            t = 'tailoring_epoch:{}, step:{}, {}time:{:4f}'.format(epoch, step, s, time.time()-start_time)
            rainbow(t)
