...
def top_k_top_p_filtering(logits, top_k, top_p, filter_value=-float("Inf")):
    '''
    解码
    :param logits: 预测结果，即预测成为词典中每个词的分数
...
def predict_one_sample(model, tokenizer, args, content):
    ...
    input_tensors = torch.tensor(input_ids, dtype=torch.long)
    token_type_tensors = torch.tensor(token_type_ids, dtype=torch.long)
    next_token_type = torch.tensor([[title_id] for _ in range(args.batch_size)], dtype=torch.long)

    if torch.cuda.is_available():
        input_tensors = input_tensors.cuda()
        token_type_tensors = token_type_tensors.cuda()
        next_token_type = next_token_type.cuda()
    ...
    with torch.no_grad():
        ...
        for _ in range(args.generate_max_len):
            ...
            # 对batch_size进行遍历，将词表中出现在序列中的词的概率进行惩罚  (之前出现过的词 给个惩罚  让其后面尽量别出现 解决重复问题)
